...
| Request frontend diblokir CORS (`No 'Access-Control-Allow-Origin'`) | Origin tidak ada di daftar efektif runtime — umum: API produksi jalan build lama (sebelum domain ditambahkan) dan/atau `.env` tidak termuat di server (file di-gitignore) | 1. **Redeploy API** dari kode terbaru. 2. Set `ALLOWED_ORIGINS=https://formup.my.id,https://www.formup.my.id` sebagai environment variable asli di host (jangan hanya mengandalkan `.env`). 3. Cek log startup: `CORS AllowedOrigins (N): ...` harus memuat domain frontend. 4. Tes preflight: `curl -i -X OPTIONS https://api.formup.my.id/api/forms -H "Origin: https://formup.my.id" -H "Access-Control-Request-Method: GET"` → harus ada header `access-control-allow-origin` |
